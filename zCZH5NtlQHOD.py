from discord import Client, File, Game, Intents, Embed
...
async def on_ready():
    ...
    await client.change_presence(activity=Game(name="nothing. type '!d help'"))
...
async def on_reaction_add(reaction, user):
    ...
    if tu is not None and tu.count >= min_votes():
        ...
        if (
            system(f"python3 {proposal_path} --bootlick")
            == APPROVED_RESPONSE_TO_DICTATOR
        ):
        ...
        await message.channel.send(f"Proposal #{index} failed.")
        proposal["status"] = "failed"
